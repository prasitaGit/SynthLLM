...
Module Info.
  ...
  Definition source_file := "swapprog.c".
...
Definition _swapnum : ident := $"swapnum".
Definition _temp : ident := $"temp".
Definition _t'1 : ident := 128%positive.

Definition f_swapnum := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_a, (tptr tint)) :: (_b, (tptr tint)) :: nil);
  fn_vars := nil;
  fn_temps := ((_temp, tint) :: (_t'1, tint) :: nil);
  fn_body :=
(Ssequence
  (Sset _temp (Ederef (Etempvar _a (tptr tint)) tint))
  (Ssequence
    (Ssequence
      (Sset _t'1 (Ederef (Etempvar _b (tptr tint)) tint))
      (Sassign (Ederef (Etempvar _a (tptr tint)) tint) (Etempvar _t'1 tint)))
    (Sassign (Ederef (Etempvar _b (tptr tint)) tint) (Etempvar _temp tint))))
|}.
...
((___compcert_va_int32,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=(Some 1); cc_unproto:=false; cc_structret:=false|})) ::
 (_swapnum, Gfun(Internal f_swapnum)) :: nil).

Definition public_idents : list ident :=
(_swapnum :: ___builtin_debug :: ___builtin_fmin :: ___builtin_fmax ::
